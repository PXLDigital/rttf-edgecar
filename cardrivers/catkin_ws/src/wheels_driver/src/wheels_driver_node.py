...
class WheelsDriverNode(object):
    def __init__(self):
        ...
        self.steering_channel     =   self.setupParam('~steering_channel',0)
        ...
        self.throttle_channel       =   self.setupParam('~throttle_channel', 1)
        ...
        self.sub_speed = rospy.Subscriber("/master/speed", Float32, self.cbSpeed, queue_size=1)
    ...
    def cbWheelsCmd(self,msg):
        ...
        #Values used for tweaking
        SPEED_MULTIPLIER = 1
        THROTTLE_INFLUENCE = 1

        DIRECTION = 0
        #If velocity is not 0, make direction -1 or 1 (forward or backward)
        if msg.velocity != 0:
            DIRECTION = msg.velocity / abs(msg.velocity)

        #Change throttle based on direction, velocity and speed
        throttle = np.clip(DIRECTION * (abs(msg.velocity * self.speed) ** .5) * SPEED_MULTIPLIER, -1, 1)
        #Change angle based on throttle
        angle = np.clip(msg.rotation * (1+abs(throttle * THROTTLE_INFLUENCE)), -1, 1)
        
        rospy.loginfo("{} | {}".format(throttle, angle) )
        self.throttle.set_speed(throttle)
